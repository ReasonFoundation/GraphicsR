...
### SCRS Unfunded Liabilities (2001-2020) w/ R (areaPlot())
                     ...
                     ticks = TRUE, sp500 = FALSE, font) 
{
  ...
  # label = c(paste("FYE 2001:",round(data.table(PERSI.debt$ava/PERSI.debt$aal)[1]*100,1),"%"), "2020 FR")
  # )
...
#debt <- areaPlot(PERSI.debt, title = "", caption = FALSE, grid = F, ticks = F, sp500 = TRUE)
#debt
```

### A History of Weakening Solvency \#2 (2001/2020) w/ R (table)
...
ual <- data.table(PERSI.debt$aal-PERSI.debt$ava)
funded <-data.table(PERSI.debt$ava/PERSI.debt$aal)
...
table[1,1] <- c("Year")
table[1,1] <- as.numeric(PERSI.debt$year[1])
table[2,1] <- as.numeric(PERSI.debt[length(PERSI.debt$year)]$year)
...
### Liabilities Growing Faster than Assets (2001-2020) w/ R (linePlot())
...
area.ual <- ualPlot(Data, title = NULL, caption = FALSE, grid = FALSE, 
                 ticks = TRUE, font = "Calibri",
                  yaxisMin = 0, yaxisMax = 60, yaxisSeq = 5,
...
table[6,2] <- as.numeric(Data$total_nc_pct)-as.numeric(Data$ee_nc_pct)
...
table <- add_header_row(table, values = "FY2022 Contributions", colwidths = 3)
...
#Modify table style using "flextable"      
 ...
 table <- width(table, j = 2:3, width = 3)
 table  
...
PERSI.debt <- Data %>% filter(plan_name == "South Carolina Retirement Systems")
PERSI.debt$year <- as.numeric(PERSI.debt$year)
PERSI.debt$ava <- as.numeric(PERSI.debt$ava)
PERSI.debt$aal <- as.numeric(PERSI.debt$aal)

sp500 <- areaPlot (PERSI.debt, title = "" , 
                   caption = FALSE, 
                   grid = FALSE, 
                   ticks = FALSE, 
                   sp500 = TRUE, 
...
### Asset Allocation (2001-2020) w/ R (custom)
...
### Debt Interest vs. Unfunded Liability Payments (2001-2020) w/ R (w/ barPlot())
...
### Change in Risk-Free Rate vs. Discount Rate (2001-2020) w/ R (linePlot())
...

{
  ...
  if (isTRUE(treasury)) {
    ...
    treasury <- read_csv(url(urlfile), col_names = TRUE, 
      na = c(""), skip_empty_rows = TRUE, col_types = NULL)
    treasury$year <- as.numeric(treasury$year)
    treasury <- treasury %>% filter(year>2000 & year < 2021)
    data$year <- as.numeric(data$year)
    treasury <- data.table(treasury %>% filter(year > min(data$year - 
      1)))
    data <- data %>% select(year, arr)
...
### Actual Change in Payroll vs. Assumption (2001-2020) w/ R (linePlot())
...
payroll <- linePlot(Data, title = NULL, caption = FALSE, grid = FALSE, 
                  treasury = FALSE, inv.returns = FALSE, ticks = TRUE, font = "Calibri",
                  yaxisMin = 0, yaxisMax = 6000, yaxisSeq = 600,
...
Data$inflation_assum <-
  PPD[PlanName == "Maryland PERS" & fy > 2000]$InflationAssumption_GASB

Data$inflation_assum <- as.numeric(Data$inflation_assum)
Data$payroll_growth_assumption <- Data$cpi.change
Data <- Data %>% select(year, inflation_assum, payroll_growth_assumption)
Data <- data.table(Data)
...
### Benefit Accrual & Retention w/ R (custom)
