...
author: "Prepared by: Pension Integrity Project at Reason Foundation January 26, 2020"
date: "02/15/2021"
output:
...
### Causes of Pension Debt (2001-2020) w/ R (custom Plotly-Image)
```{r}
...
DPF.data$ava_return <- ava_returns
DPF.data$year <- as.numeric(DPF.data$year)
...
graph <- linePlot(DPF.data, title = "", inv.returns = TRUE, treasury = FALSE,
                  font = "Calibri", yaxisMin = -27, yaxisMax = 27, yaxisSeq = 3,
...
assets <- ggplot(PERSI.assets) + geom_area(aes(x = year, y = value, fill = variable, group = variable), position="fill")+
  ...
  scale_x_continuous(labels = function(x) paste0(x, ""), name = "",
                     breaks = seq(2001, 2020, by = 2), limits = c(2001, 2020), expand=c(0,0))+
  theme_bw()+
  plotTheme+
