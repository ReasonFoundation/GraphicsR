...
### A History of Weakening Solvency (2001-2020) w/ R (areaPlot())
...
#View(Data)
PERSI.debt <- data.table(Data) %>% filter(plan_name == "Idaho Public Employee Retirement System") 
  ...
  ticks = TRUE, sp500 = FALSE, font) 
{
  ...
  y_minimum <- as.numeric(min(
    if (!isTRUE(sp500)) {
    graph$uaal
  } else {
    0
  }))
  y_maximum <- as.numeric(max(
    if (!isTRUE(sp500)) {
    graph$uaal
  } else {
    graph$SP500
  ...
  lab3 = NULL, lab4 = NULL, lab5 = NULL) 
{
  ...
  if (isTRUE(treasury)) {
    ...
    diff <- (data[year == min(data$year)]$dr - data[year == 
...
PERSI.data$payroll <- as.numeric(PERSI.data$payroll)
PERSI.data$payroll_growth_assumption <- as.numeric(PERSI.data$payroll_growth_assumption)
payroll.growth <- PERSI.data$payroll_growth_assumption[1]
PERSI.data <- PERSI.data %>% select(year, payroll, payroll_growth_assumption)
PERSI.data <- data.table(PERSI.data) 

PERSI.data$payroll_growth_assumption[1] <- PERSI.data$payroll[1]
for(i in (2:length(PERSI.data$payroll_growth_assumption))){
  PERSI.data$payroll_growth_assumption[i] <- PERSI.data$payroll_growth_assumption[i-1]*(1+payroll.growth)
...
payroll <- linePlot(PERSI.data, title = NULL, caption = FALSE, grid = FALSE, 
...
PERSI.data$inflation_assum <- as.numeric(PERSI.data$inflation_assum)
PERSI.data <- PERSI.data %>% select(year, inflation_assum, payroll_growth_assumption)
PERSI.data <- data.table(PERSI.data)

PERSI.data$inflation_assum <- PERSI.data$payroll_growth_assumption

PERSI.data$inflation_assum[18:20] <- c(0.025, 0.025, 0.023)

PERSI.data$payroll_growth_assumption <-
c(
0.023325359,
0.012857978,
0.023081362,
...
inflation <- linePlot(PERSI.data, title = NULL, caption = FALSE, grid = FALSE, 
                  treasury = FALSE, inv.returns = FALSE, ticks = TRUE, font = "Calibri",
                  yaxisMin = -1, yaxisMax = 6, yaxisSeq = 1,
                  yaxisScale = 100, format = "%", str = 60,
                  labelY = "Inflation %", lab1 = "Assumed Inflation (by PERSI)",
