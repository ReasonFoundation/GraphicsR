...
output: 
  powerpoint_presentation:
    reference_doc: template_III.pptx
---
...
### A History of Weakening Solvency (2001-2019) w/ R (debtPlot())
...
############
debtPlot <- function(data, title = NULL, caption = FALSE, grid = FALSE, ticks = TRUE, sp500 = FALSE, font) {
  ...
  y_minimum <- as.numeric(min(if(!isTRUE(sp500)){graph$uaal}else{graph$SP500}))
  ...
  ggplot2::ggplot(graph,ggplot2::aes(x = graph$year)) +
    ...
    # colors assigned to pos, neg
        ...
        values = c("negative" = paste(palette_reason$Green),#Referenced Color Palette
                   "positive" = paste(palette_reason$Red)))+    # sets the y-axis scale
...
debt <- debtPlot(PERSI.debt, caption = F, grid = F, ticks = F, sp500 = F, font = "Calibri")
...
### A Mountain fo Debt Plot w/ R (Saved Img)
![](PERSI.debptPlot2.jpeg)

### A History of Weakening Solvency (2001-2019) w/ R (Saved Img)
